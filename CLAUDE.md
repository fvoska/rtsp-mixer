...
- **Never accept GitHub's auto-generated PR title or description.** GitHub defaults them to the first commit on the branch (often a `test:`/`chore:` setup commit), which misclassifies the change for release-please. Always write the PR title and description deliberately: the title uses the type of the overall change (a feature PR is `feat(...)` even if its first commit is a test commit), and the description summarizes the whole diff. When a PR was opened with the defaults, fix the title/description instead of leaving them.
